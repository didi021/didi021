...
num_node_ly3 = 2048
num_epochs = 5000
batch_size_const = 100

# dataset processing
x_para = pd.read_csv(data_dir + "para.txt", header=None, encoding='utf-8')
y_trans = pd.read_csv(data_dir + "trans.txt", header=None, encoding='utf-8')
X_old = x_para.iloc[:, :].values
Y_old = y_trans.iloc[:, :].values
...
model.add(tf.keras.layers.Dense(units=num_node_ly1, input_shape=(5,), activation='relu'))
model.add(tf.keras.layers.Dense(units=num_node_ly2, activation='relu'))
model.add(tf.keras.layers.Dense(units=num_node_ly3, kernel_regularizer=tf.keras.regularizers.l2(0.0000001),
                                activation='relu'))
